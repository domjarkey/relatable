...
test_that("fatal errors reported correctly",	{
  A1 <- c("a", "b", "c")
  B1 <- c(1, 2, 3)
  # Invalid error_response given
  expect_error(relate(A1, A1, B1, error_response = "albatross"),
    '"albatross" is not a valid input for error_response. Use "ignore", "warn", or "throw".')
  # atomic = TRUE when min_one_y_per_x = FALSE
  expect_error(relate(A1, A1, B1, atomic = TRUE, relation_type = NULL),
  ...
  expect_error(relate(A1, A1, B1, heterogeneous_outputs = TRUE, relation_type = NULL),
    "Many-to-many and one-to-many relations can only return list vectors. Use atomic = FALSE.")
  # Invalid inputs return errors when allow_default = FALSE
  expect_error(relate("d", A1, B1, allow_default = FALSE, atomic = FALSE, relation_type = NULL),
    "d does not have a valid mapping to an element in the codomain.")
})

test_that("error_response options",	{
  A1 <- c("a", "b", "c")
  B1 <- c(1, 2, 2)
  # Ignore
  expect_identical(relate(A1, A1, B1, relation_type = "bijection", error_response = "ignore"),
    c(1, 2, 2))
  # Warn
  expect_warning(relate(A1, A1, B1, relation_type = "bijection", error_response = "warn"),
    "Vector B contains duplicate elements which may map from different elements in the domain.")
  # Throw
  expect_error(relate(A1, A1, B1, relation_type = "bijection", error_response = "throw"),
    "Vector B contains duplicate elements which may map from different elements in the domain.")
})

test_that("report conflicts between mappings and restrictions", {
  A1 <- c("a", "a", "c", "d", "e")
  A2 <- c("a", "b", "c", "d")
  B1 <- c(1, 2, 2, 3, 4)
  B2 <- c(1, 2, 3, 4)
